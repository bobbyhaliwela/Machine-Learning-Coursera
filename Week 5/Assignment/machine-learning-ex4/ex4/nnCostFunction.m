...
% Hidden layer
z2 = a1 * Theta1';
a2 = sigmoid(z2); % Apply the sigmoid activation function
a2 = [ones(m, 1) a2]; % Add bias to a2

% Output layer
z3 = a2 * Theta2';
a3 = sigmoid(z3); % Apply the sigmoid activation function
hThetaX = a3;

% COST FUNCTIONS

% Compute non-regularized cost function
J = 1/m * sum(sum(-1 * y_matrix .* log(hThetaX) - (1 - y_matrix) .* log(1 - hThetaX)));
